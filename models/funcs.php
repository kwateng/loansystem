...
//Check whether guarantor exist
function gurantorExists($gurantorId)
{
    global $mysqli, $db_table_prefix;
...
//Checks if a user is a accountant
...
//Checks if a user is a admin
function isEmployee($user_id)
{
    global $mysqli,$db_table_prefix;
    $stmt = $mysqli->prepare("SELECT user_name
		FROM ".$db_table_prefix."users
		WHERE
		id = ? AND role='Employee'
		LIMIT 1");
    $stmt->bind_param("i", $user_id);
    $stmt->execute();
    $stmt->store_result();
...
function securePage($uri){
    ...
    return isPageSucure($page);
}

function securePageForUser($page) {
    return isPageSucure($page);
}

function isPageSucure($page) {
