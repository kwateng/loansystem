...
class User 
{
    ...
    public $mobile_no;
    public $land_no;
	function __construct($user,$display,$pass,$email,$designation,$salary,$mobile_no,$land_no,$service,$employee_id,$address,$full_name,$role)
	{
	...
	public function userAddUser()
	{
		...
		if($this->status)
		{
			...
			if(!$this->mail_failure)
			{
				...
				$stmt->bind_param("sssssidssisssss", $this->username, $this->displayname, $secure_pass, $this->clean_email, $this->activation_token, $this->user_active,
                    $this->salary,$this->mobile_no,$this->land_no,$this->employee_id,$this->service,$this->address,$this->dsignation,$this->full_name,$this->role);
				$stmt->execute();
				...
				$stmt->bind_param("si", $inserted_id,$permission_level);
				$stmt->execute();
				$stmt->close();
			}
		}
